...
iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 3389 -j DNAT --to-destination $(nmap -v  192.168.121.1/24 -p 3389 | grep "Scanning 192.168.121." | cut -c 10-24)
iptables -t nat -A POSTROUTING -o virbr1 -p tcp --dport 3389 -d $(nmap -v  192.168.121.1/24 -p 3389 | grep "Scanning 192.168.121." | cut -c 10-24) -j SNAT --to-source 192.168.121.1
